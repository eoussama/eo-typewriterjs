...
const HELP_SECTIONS: readonly THelpSection[] = [
  {
    ...
    rows: [
      ...
      ["stringRenderer()", "Create a headless string renderer"],
  ...
  },
  {
    ...
    rows: [
      ...
      ["by", "\"char\" | \"word\" | \"line\" | \"grapheme\" | \"whole\" - advance unit (default: \"char\"); or { unit, amount } to consume multiple units per step"],
      ...
      ["before", "Hook - fires once per step (before each insertion)"],
      ["after", "Hook - fires once per step (after each insertion)"],
  ...
  },
  {
    ...
    rows: [
      ...
      ["by", "\"char\" | \"word\" | \"line\" | \"grapheme\" - unit per step (numeric counts only, default: \"char\"); or { unit, amount } object form"],
      ["interval", "Delay in ms between each deletion step (numeric counts only, default: 50)"],
      ["cursor", "\"main\" or an array of cursor IDs"],
      ["before", "Hook - fires once per step (before each deletion); fires once total for boundary operands"],
      ["after", "Hook - fires once per step (after each deletion); fires once total for boundary operands"],
      ["audio", "false to silence, or { volume, sfx } to override audio for this command"],
    ],
  },
  {
  ...
  },
  {
    ...
    rows: [
      ...
      ["range", "{ from, to } - absolute indices, or \"selection\" to use the cursor's active selection (selection is cleared after the style is applied)"],
  ...
  },
  {
    ...
    rows: [
      ["range", "{ from, to } - absolute indices, or \"selection\" to unstyle the cursor's active selection (selection is cleared after unstyle is applied)"],
      ["cursor", "Whose selection to use when range is \"selection\" (default: \"main\")"],
  ...
  },
  {
    ...
    rows: [
      ["fn", "({ state, stepIndex, stepCount, unit, signal }) => void | Promise<void> - if a Promise is returned, playback suspends until it settles"],
      ["- context: state", "Current TTypewriterState snapshot"],
      ["- context: signal", "AbortSignal - aborted when tw.cancel() is called"],
      ["note", "Runtime-only: fires only during play()/replay(); has no effect during seek(), stepForward(), or stepBackward()"],
  ...
  },
  {
    ...
    rows: [
      ...
      ["- context: unit", "The text chunk typed or deleted in this step; null for instant, boundary, and timing-only commands"],
      ["- context: signal", "AbortSignal - aborted when tw.cancel() is called"],
    ],
  },
  {
  ...
  },
  {
    ...
    rows: [
      ...
      ["cursor: { animation: \"blink\" }", "Cursor animation: \"blink\" | \"none\" | TCursorAnimationOptions object ({ name, duration, timingFunction, iterationCount, ... })"],
      ["cursor: { visible: false }", "Hide the cursor initially"],
      ["cursor: { className: \"my-cls\" }", "Extra CSS class(es) applied to the cursor <span> element"],
