...
const HELP_SECTIONS: readonly THelpSection[] = [
  ...
  },
  {
    ...
    desc: "Delete units relative to the cursor. Positive count deletes forward; negative deletes backward; zero deletes the whole text.",
    rows: [
      ["count", "Number of units — positive = forward, negative = backward, 0 = whole text"],
      ["by", "\"char\" | \"word\" | \"line\" | \"grapheme\" | \"whole\" — unit per step (default: \"char\")"],
  ...
  },
  {
    ...
    desc: "Instantly move a cursor relative to its current position. Positive = right, negative = left, zero = no-op.",
    rows: [
      ["offset", "Units to move — positive = right, negative = left, 0 = no-op"],
      ["by", "\"char\" | \"word\" | \"line\" | \"grapheme\" — unit (default: \"char\")"],
  ...
  },
  {
    ...
    desc: "Extend a selection from the current cursor position. Positive = forward, negative = backward, 0 = whole text.",
    rows: [
      ["count", "Number of units — positive = forward, negative = backward, 0 = whole text"],
      ["by", "\"char\" | \"word\" | \"line\" | \"grapheme\" | \"whole\" — selection unit (default: \"char\")"],
      ["cursor", "\"main\" or an array of cursor IDs"],
  ...
  },
  {
    ...
    rows: [
      ...
      ["audio: { typing: { strategy: EAudioStrategy.SHUFFLE_BAG } }", "Per-channel voice strategy: RANDOM, SHUFFLE_BAG, ROUND_ROBIN"],
      ...
      ["audio: { voices: { name: { samples: [\"...\"] } } }", "Named voice pack; each voice has a samples array of audio URLs"],
      ["audio: { typing: { voice: \"name\" } }", "Target a named voice for a specific channel"],
      ...
      ["type/delete: { audio: { voice: \"name\" } }", "Use a specific named voice for one command"],
    ],
  },
  {
    id: "cursor",
